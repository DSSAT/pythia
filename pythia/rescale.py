...
def get_vector_resolution(file_path):
    with fiona.open(file_path, 'r') as src:
        ...
        if (min_x_dist not in (None, 0)) and (min_y_dist not in (None, 0)): #if min_x_dist is not None and min_y_dist is not None:
            #return [round(min_x_dist, 6), round(min_y_dist, 6)] # acredito que o problema seja que o valor de y é tao pequeno que usando o round com 6 casas ele vira 0
...
#                 x_dist = abs(src[i]['geometry']['coordinates'][0] - src[i + 1]['geometry']['coordinates'][0]) #is the x value of the geometry point 
#                 y_dist = abs(src[i]['geometry']['coordinates'][1] - src[i + 1]['geometry']['coordinates'][1]) #is the y value of the geometry point 
#                 if 0 < x_dist:
