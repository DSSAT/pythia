...
def _validate_config(config):
    ...
    values_iter = itertools.chain.from_iterable([list(config["default_setup"].values())] + [list(r.values()) for r in config["runs"]])
    rasters =  list(set([pythia.functions.extract_raster(raster) for raster in list(filter(lambda x: "raster::" in str(x), values_iter))]))
    cached_crs = None
    for r in rasters:
