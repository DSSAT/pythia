    ...
    try:
        ...
        if cached_crs:
            if cached_crs != current_crs:
                logging.error(
                    "CRS mismatch for file {}. {} -> {}. Please reconcile.".
                    format(raster_file, cached_crs['init'],
                           current_crs['init']))
    ...
    rasters = list(
        set([
            pythia.functions.extract_raster(raster) for raster in list(
                filter(lambda x: "raster::" in str(x), values_iter))
        ]))
    cached_crs = None
    for r in rasters:
...
def _set_run_workdir(run, root, idx):
    run_dir = os.path.join(root,
                           "{}".format(run.get("name", "run_{}".format(idx))))
...
def _merge_runs(config):
    ...
    runs = [
        _set_run_workdir(r, config.get("workDir", "."), i)
